...
public:
    ...
    virtual int32_t RegisterApplicationStateObserver(
        const sptr<IRemoteObject> &observer, const std::vector<std::string> &bundleNameList) = 0;
    virtual int32_t UnregisterApplicationStateObserver(const sptr<IRemoteObject> &observer) = 0;
    virtual int32_t GetForegroundApplications(std::vector<AppStateData> &list) = 0;
    virtual int32_t DisconnectAppMgrService() = 0;
};
