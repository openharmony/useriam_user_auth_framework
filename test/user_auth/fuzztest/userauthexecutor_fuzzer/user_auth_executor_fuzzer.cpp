...
public:
    ...
    int32_t Finish(
        uint64_t scheduleId, int32_t srcType, int32_t resultCode, std::shared_ptr<AuthAttributes> finalResult) override
    {
        return fuzzParcel_->ReadInt32();
    }
...
void FillIAuthAttributes(std::shared_ptr<Parcel> parcel, std::shared_ptr<AuthAttributes> &attributes)
{
    bool fillNull = parcel->ReadBool();
    if (fillNull) {
        attributes = nullptr;
    ...
    attributes = MakeShared<AuthAttributes>();
    attributes->SetUint64Value(AUTH_TEMPLATE_ID, parcel->ReadUint64());
    attributes->SetUint64Value(AUTH_CALLER_UID, parcel->ReadUint64());
    attributes->SetUint32Value(AUTH_PROPERTY_MODE, parcel->ReadUint32());
    std::vector<uint64_t> templateIdList;
    FillFuzzUint64Vector(*parcel, templateIdList);
    attributes->GetUint64ArrayValue(AUTH_TEMPLATE_ID_LIST, templateIdList);
    attributes->SetUint64Value(AUTH_CALLER_UID, parcel->ReadUint64());
    attributes->SetUint32Value(AUTH_SCHEDULE_MODE, parcel->ReadUint32());
}
...
void FuzzFrameworkOnBeginExecute(std::shared_ptr<Parcel> parcel)
{
    ...
    std::vector<uint8_t> publicKey;
    FillFuzzUint8Vector(*parcel, publicKey);
    std::shared_ptr<AuthAttributes> commandAttrs = nullptr;
    FillIAuthAttributes(parcel, commandAttrs);
...
void FuzzFrameworkOnEndExecute(std::shared_ptr<Parcel> parcel)
{
    ...
    uint64_t scheduleId = parcel->ReadUint64();
    std::shared_ptr<AuthAttributes> consumerAttr = nullptr;
    FillIAuthAttributes(parcel, consumerAttr);
...
void FuzzFrameworkOnSetProperty(std::shared_ptr<Parcel> parcel)
{
    ...
    std::shared_ptr<AuthAttributes> properties = nullptr;
    FillIAuthAttributes(parcel, properties);
...
void FuzzFrameworkOnGetProperty(std::shared_ptr<Parcel> parcel)
{
    ...
    std::shared_ptr<AuthAttributes> conditions = nullptr;
    FillIAuthAttributes(parcel, conditions);
    std::shared_ptr<AuthAttributes> values = nullptr;
    FillIAuthAttributes(parcel, values);
