...
HWTEST_F(UserIdmServiceTest, UserIdmServiceDelCredential004, TestSize.Level0)
{
    UserIdmService service(123123, true);
    int32_t testUserId = 1548865;
    uint64_t testCredentialId = 23424;
    std::vector<uint8_t> emptyAuthToken;

    sptr<MockIdmCallback> testCallback(new (std::nothrow) MockIdmCallback());
    EXPECT_NE(testCallback, nullptr);
    EXPECT_CALL(*testCallback, OnResult(INVALID_PARAMETERS, _)).Times(1);

    EXPECT_EQ(service.DelCredential(testUserId, testCredentialId, emptyAuthToken, testCallback),
        INVALID_PARAMETERS);
}

HWTEST_F(UserIdmServiceTest, UserIdmServiceDelCredential005, TestSize.Level0)
{
    UserIdmService service(123123, true);
    int32_t testUserId = 1548865;
    uint64_t testCredentialId = 23424;
    std::vector<uint8_t> emptyAuthToken;
    sptr<IIamCallback> nullCallback(nullptr);

    EXPECT_EQ(service.DelCredential(testUserId, testCredentialId, emptyAuthToken, nullCallback),
        INVALID_PARAMETERS);
}

HWTEST_F(UserIdmServiceTest, UserIdmServiceDelCredential006, TestSize.Level0)
{
    UserIdmService service(123123, true);
    int32_t testUserId = 1548865;
    uint64_t testCredentialId = 23424;
    uint64_t testCustCredentialId = 56789;
    std::vector<uint8_t> emptyAuthToken;
    ...
    auto mockHdi = MockIUserAuthInterface::Holder::GetInstance().Get();
    EXPECT_NE(mockHdi, nullptr);
    EXPECT_CALL(*mockHdi, GetCredentialById(testCustCredentialId, _))
        .WillOnce(
            [](uint64_t credentialId, HdiCredentialInfo &info) {
                info.authType = static_cast<HdiAuthType>(128);
                info.credentialId = credentialId;
                return HDF_SUCCESS;
            }
        );
    EXPECT_CALL(*mockHdi, DeleteCredential(_, testCustCredentialId, _, _))
        .WillOnce(
            [](int32_t userId, uint64_t credentialId, const std::vector<uint8_t> &authToken,
                HdiCredentialOperateResult &operateResult) {
                operateResult.operateType = HdiCredentialOperateType::CREDENTIAL_DELETE;
                HdiCredentialInfo credentialInfo = {};
                credentialInfo.authType = static_cast<HdiAuthType>(128);
                credentialInfo.credentialId = 10;
                credentialInfo.executorIndex = 20;
                credentialInfo.executorMatcher = 30;
                credentialInfo.executorSensorHint = 40;
    ...
    IpcCommon::AddPermission(MANAGE_USER_IDM_PERMISSION);
    EXPECT_EQ(service.DelCredential(testUserId, testCustCredentialId, emptyAuthToken, testCallback), SUCCESS);
    IpcCommon::DeleteAllPermission();
}

HWTEST_F(UserIdmServiceTest, UserIdmServiceDelCredential007, TestSize.Level0)
{
    UserIdmService service(123123, true);
    int32_t testUserId = 1548865;
    uint64_t testCredentialId = 23424;
    ...
    auto mockHdi = MockIUserAuthInterface::Holder::GetInstance().Get();
    EXPECT_NE(mockHdi, nullptr);
    EXPECT_CALL(*mockHdi, GetCredentialById(testCredentialId, _))
        .WillOnce(Return(GENERAL_ERROR));
