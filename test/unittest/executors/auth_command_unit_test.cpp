...
void AuthCommandUnitTest::SetUpTestCase()
{
}

void AuthCommandUnitTest::TearDownTestCase()
{
}

void AuthCommandUnitTest::SetUp()
{
}

void AuthCommandUnitTest::TearDown()
{
}

HWTEST_F(AuthCommandUnitTest, AuthCommand_OnResultTest_001, TestSize.Level0)
{
...
HWTEST_F(AuthCommandUnitTest, AuthCommand_OnResultTest_002, TestSize.Level0)
{
    ...
    EXPECT_CALL(*messenger, Finish(_, _, _, _))
        .Times(Exactly(1))
        .WillOnce([](uint64_t scheduleId, int32_t srcType, int32_t resultCode,
                      std::shared_ptr<Attributes> finalResult) { return USERAUTH_ERROR; });
    auto executor = Common::MakeShared<Executor>(nullptr, nullptr, 3);
    ASSERT_NE(executor, nullptr);
    auto command = Common::MakeShared<AuthCommand>(executor, testScheduleId, nullptr, messenger);
    ASSERT_NE(command, nullptr);
