...
HWTEST_F(UserAuthClientTest, UserAuthClientRegistUserAuthSuccessEventListenerSuccess, TestSize.level0)
{
    ...
    EXPECT_CALL(*obj, IsProxyObject()).WillRepeatedly(Return(true));
    ON_CALL(*obj, SendRequest)
        .willByDefault([&service](uint32_t code, MessageParcel &data, MessageParcel &reply, NessageOption &option) {
            service->OnRemoteRequest(code, data, reply, option);
            return OHOS::NO_ERROR;
        })
    ...
    listenerImpl->OnNotifyCredChange(userId, authType, callerType, eventInfo);
    uint32_t ipcCode = 1;
    ...
    EXPECT_EQ(ret, SUCCESS);
    ON_CALL(*service, UnRegistenerUserAuthSuccessEventListener)
        .WillByDefault(
            [](const sptr<IEventListenerCallback> &listener) {
                return SUCCESS;
            }
...
HWTEST_F(UserAuthClientTest, UserAuthClientInitIpcRemoteAuthParam, TestSize.Level0)
{
    ...
    EXPECT_NO_THROW(UserAuthClientImpl::Instance().InitIpcRemoteAuthParam(remoteAuthPram, ipcRemoteAuthParam));
}
